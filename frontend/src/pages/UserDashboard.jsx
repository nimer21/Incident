...
const UserDashboard = () => {
    ...
    useEffect(() => {
        const fetchIncidents = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:5005/api/users/${user.userId}/incidents`,
        ...
        const handleViewIncident = async (id) => {
            try {
                const response = await axios.get(`http://localhost:5005/api/incidents/get-incident/${id}`, {
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
                    ...
                    <tbody>
                        {incidents.map((incident) => (
                            ...
                            className="hover:bg-gray-50 transition duration-150 ease-in-out">
                                ...
                                <td className={`border p-3 text-${incident.severity.toLowerCase()}`}>
                                {incident.severity}
                                </td>
                                <td className="border p-2">{new Date(incident.createdAt).toLocaleDateString()}</td>
            ...
            {modalOpen && selectedIncident && (
                <IncidentDetailsModal selectedIncident={selectedIncident} setSelectedIncident={setSelectedIncident} onClose={handleCloseModal} />
            )}
